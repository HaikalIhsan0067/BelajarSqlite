...
public class DbHelper extends SQLiteOpenHelper {
    ...
    public boolean simpan(MahasiswaModel mm){
    ...
    public ArrayList<MahasiswaModel> list(){
        ...
        if (cursor.getCount() >0){
            while (cursor.moveToNext()){
                ...
                MahasiswaModel mm = new MahasiswaModel(id, nama, nim, noHp);
                mhsList.add(mm);
            }
        }
        return mhsList;
    ...
    public boolean ubah(MahasiswaModel mm){
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put(COLUMN_NAMA, mm.getNama());
        cv.put(COLUMN_NIM, mm.getNim());
